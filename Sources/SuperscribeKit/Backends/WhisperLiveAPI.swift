...
enum WhisperLiveAPI {
    @TaskLocal internal static var invocation: (@Sendable (whisper_full_params, [Float]) -> Int32)?

    @TaskLocal internal static var testState = TestDependencyStorage(TestState())

    internal struct TestState {
        var testSkipContextRelease = false
    }

    /// When `true`, `releaseContext` is a no-op (covers managed deinit without a real model).
    internal static var testSkipContextRelease: Bool {
        get { return Self.testState[\.testSkipContextRelease] }
        set { Self.testState[\.testSkipContextRelease] = newValue }
    }

    static func releaseContext(_ context: OpaquePointer) -> Void {
        if Self.testSkipContextRelease == true { return }
        whisper_free(context)
    }

    static func initContext(from binPath: String, ctxParams: whisper_context_params) -> OpaquePointer? {
        return whisper_init_from_file_with_params(binPath, ctxParams)
    }

    static func initState(context: OpaquePointer, modelId: String) -> OpaquePointer? {
        if (WhisperBackend.isStubModel(modelId)) == true, let injected = WhisperBackend.testWhisperStatePointer {
            return injected
        }
        if (Self.shouldUseStubAPI(for: modelId)) == true {
            return context
        }
    ...
    static func releaseState(
        ...
        if (WhisperBackend.isStubModel(modelId)) == true, WhisperBackend.testWhisperStatePointer != nil { return }
    ...
    static func runFull(
        ...
        if useStubAPI == true {
            if let invocation = Self.invocation { return invocation(params, samples) }
            return 0
        }
    ...
    static func segmentCount(from state: OpaquePointer, modelId: String) -> Int {
        if (WhisperBackend.isStubModel(modelId)) == true, let segments = WhisperBackend.testWhisperAPISegments {
    ...
    static func tokenCount(from state: OpaquePointer, segment: Int, modelId: String) -> Int {
        if (WhisperBackend.isStubModel(modelId)) == true,
    ...
    private static func shouldUseStubAPI(for modelId: String) -> Bool {
        return WhisperBackend.testWhisperAPISegments != nil && WhisperBackend.isStubModel(modelId)
    }
