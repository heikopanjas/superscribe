...
public struct TokenAccumulator: Sendable {
    ...
    public mutating func accept(token: String, start: TimeInterval, end: TimeInterval) -> Void {
        let isNewWord = token.hasPrefix("▁") || token.hasPrefix(" ")

        if isNewWord == true && self.currentText.isEmpty == false {
            self.flush()
        }
        ...
        if self.currentText.isEmpty == true {
            self.wordStart = start
        }
    ...
    private mutating func flush() -> Void {
        guard self.currentText.isEmpty == false else { return }
        self.words.append(
            TimedWord(
